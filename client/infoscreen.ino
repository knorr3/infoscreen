 ...
 */
     ...
     for (JsonVariant entry : entries)
     {
         ...
         if (entry["type"] == "calendar" && entry["status"] == "ok")
         {
             ...
             for (JsonVariant item : data)
             {
                 ...
                 cal[i].today = item["today"];
                 cal[i].allDay = item["allDay"];
                 i++;
             }
         }
     }
 ...
 void drawRtcDateTime()
 {
     ...
     writeln((GFXfont *)&CourB36, buf, &cursor_x, &cursor_y, framebuffer);
 
     cursor_x = 50;
     cursor_y = 120;
     strftime(buf, 64, "%a %d.%m", &timeinfo);
     writeln((GFXfont *)&Cour20, buf, &cursor_x, &cursor_y, framebuffer);
 }
 ...
 void drawWeather()
 {
     ...
     for (int i = 0; i < MAX_WEATHER_DATA; i++)
     {
         ...
         int percip_int = (int)round(weather[i].precip);
 
         snprintf(buf, 128, "%d°C", temp_int);
         int temp_length = strlen(buf) - 3;
 
         cursor_x = 400 - 7 * temp_length + i * 205;
         cursor_y = 50;
         area.x = 400 + i * 200;
         writeln((GFXfont *)&CourB28, buf, &cursor_x, &cursor_y, framebuffer);
         if (percip_int == 0) { 
             snprintf(buf, 128, "--", percip_int);
             int percip_length = strlen(buf) - 3;
 
             cursor_x = 410 + i * 200;
             cursor_y = 80;
 
             writeln((GFXfont *)&Cour16, buf, &cursor_x, &cursor_y, framebuffer);
         } else {
             snprintf(buf, 128, "%dmm", percip_int);
             int percip_length = strlen(buf) - 3;
 
             cursor_x = 410 - 10 * percip_length + i * 200;
             cursor_y = 80;
             writeln((GFXfont *)&Cour16, buf, &cursor_x, &cursor_y, framebuffer);
 
             cursor_x = 388 + i * 200;
             cursor_y = 55;
             epd_draw_rect(cursor_x, cursor_y, 100, 90, 0, framebuffer);
         }
         ...
         epd_copy_to_framebuffer(area, (uint8_t *)weatherIcons[iconIdx], framebuffer);
     }
 }
 ...
 void drawDb()
 {
     ...
     for (int i = 0; i < MAX_DB_DATA; i++)
     {
         ...
         {
             snprintf(buf, 128, "--");
             writeln((GFXfont *)&CourB20, buf, &cursor_x, &cursor_y, framebuffer);
         }
 ...
 void drawCal()
 {
     ...
     for (int i = 0; i < MAX_CAL_DATA; i++)
     {
         ...
         if (sscanf(cal[i].date, "%d-%d-%dT%d:%d:%dZ", &year, &month, &day, &hour, &minute, &second) == 6)
         {
             if (cal[i].allDay)
             {
                 // Format the output as "DD.MM - all day"
                 snprintf(buf, 128, "%02d.%02d - all day", day, month);
             }
             else
             {
             ...
             // If a event exists today and the current event is not today, draw a line above it
             if (todayExists && !cal[i].today)
             {
                 epd_draw_hline(EPD_WIDTH/2, cursor_y - 50, EPD_WIDTH/2, 0, framebuffer);
                 todayExists = false;
             }
         }
         else
 ...
 void setup()
 {
     ...
     sniprintf(mqttRequestDataMsg, MISC_MSG_SIZE, "{\"type\":\"request\",\"data\":[{\"name\":\"weather\",\"count\":%d},{\"name\":\"db\",\"count\":%d},{\"name\":\"calendar\",\"count\":%d}]}", MAX_WEATHER_DATA, MAX_DB_DATA, MAX_CAL_DATA);
